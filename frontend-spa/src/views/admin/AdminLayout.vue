<script setup>
  ...
  <template>
    <div class="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300">
      ...
      <div class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-colors duration-300">
        <div>
          <div class="flex h-16 items-center px-6 border-b border-slate-200 dark:border-slate-800">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Agency SaaS</h1>
          </div>
        ...
        <div class="border-t border-slate-200 dark:border-slate-800 p-4 space-y-3">
           <div class="flex items-center justify-between px-2">
               <div class="flex flex-col">
                  ...
                  <span class="text-xs text-slate-500 dark:text-slate-500 truncate max-w-[120px]" :title="auth.user?.email">
                    {{ auth.user?.email }}
                  </span>
               </div>
          ...
          <button @click="logout" class="w-full flex items-center justify-center px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/10 dark:hover:text-red-300 rounded-lg transition-colors duration-200">
