<script setup>
    ...
    const availableStatuses = computed(() => lead.value?.crm_config?.statuses || [{slug:'new',label:'New'},{slug:'contacted',label:'Contacted'},{slug:'closed',label:'Closed'}])
                ...
                <div class="card-body">
                    <div class="flex flex-wrap gap-6 mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                        ...
                        <div v-if="lead.form">
                            ...
                            <span class="text-sm font-medium text-blue-600 dark:text-blue-400">{{ lead.form.name }}</span>
                        </div>
                    </div>
                    <div v-if="lead.payload" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div v-for="(value, key) in lead.payload" :key="key">
                            ...
                            <dd class="text-sm text-slate-900 dark:text-white break-words">{{ (Array.isArray(value) ? value.join(', ') : value) || '-' }}</dd>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body space-y-4">
                            <div class="form-group">
                                ...
                                <select v-model="lead.status" @change="updateLead" class="form-select">
                                    <option v-for="s in availableStatuses" :key="s.slug" :value="s.slug">{{ s.label }}</option>
                                </select>
                                <div class="mt-2"><span :class="['badge', statusBadge]">{{ availableStatuses.find(s=>s.slug===lead.status)?.label || lead.status }}</span></div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="mt-2"><span :class="['badge', tempBadge]">{{ lead.temperature }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
